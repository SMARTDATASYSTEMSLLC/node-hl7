...
function Encoder2() {
    var COMPONENT_SEPARATOR, REPEAT_SEPARATOR, DEFAULT_SEPARATORS, FIELD_SEPARATOR, NEW_LINE;

    FIELD_SEPARATOR = '|';
    COMPONENT_SEPARATOR = '^';
    REPEAT_SEPARATOR = '~';
    NEW_LINE = '\r';
    DEFAULT_SEPARATORS = "" + COMPONENT_SEPARATOR + REPEAT_SEPARATOR + "\\&";
            ...
            _.each(value, function (val){
                ...
                for (var i = 1; i <= len; i++) {
                    if (_.isString(val[name + '.' + i]) || !val[name + '.' + i]){
                        ...
                        result.push(_.map(val[name + '.' + i], function (subvalue){
                            return subvalue;
                        }).join(REPEAT_SEPARATOR));
                    }
                }
            });
