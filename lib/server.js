...
function Server(host, port, options) {
    ...
    var applicationErrorACK = String.fromCharCode(11) + 'MSH|^~\\&|MediDoc|MedidocError|TESTSYSTEM|TESTFACILITY|20121217158325.194-0700||ACK^RO1|101|T|2.6\nMSA|AE|17'+ String.fromCharCode(13,28,13);
    ...
    this.socket = net.createServer({ allowHalfOpen: false}, function(socket){
        ...
        function createEL7AckForMessageNumber(messageNumber) {
            var ack = String.fromCharCode(11) + 'MSH|^~\\&|MediDoc||HSTPathways||' + generateTimeStamp() + '|1100|ACK^RO1|'+ messageNumber +'|P|2.3\rMSA|AA|' + messageNumber + '|MSG Received Successfully'+ String.fromCharCode(13,28,13);
            return ack;
        }
        ...
        function generateTimeStamp() {
            ...
            var timestamp = `${now.getUTCFullYear().toString()}${month}${date}${hours}${minutes}${seconds}.${now.getUTCMilliseconds().toString()}+0000`;
            return timestamp;
        }
        ...
        socket.on('data', function(packet) {
            ...
            if (packet.indexOf('<?xml') === -1) {
                if (packet.indexOf(String.fromCharCode(28, 13, 11)) > -1) {
                    packet = packet.split(String.fromCharCode(28, 13, 11));
                } else {
                    packet = [packet];
                }

                packet.forEach(p => hl7.toXml(p, function (error, xml) {
                    if (error) {return handleError(error);}
                    handleXML(xml, true, p);
                }));
            } else {
                handleXML(packet.trim(), false, packet.trim());
            }
        });
